...
#create trial labels
all_labels = np.repeat(np.arange(DAT['Del_trials'].shape[0]),n_rep)

# let's try only yes sound no sound
# #window=[-0.5 1.6]
# new window from =10ms to 1s
noStim_data = resampled_motSVD[:,:50,40:-60]
Vis_data = resampled_motSVD[:,-150:-100,40:-60]

new_resampled_motSVD = np.concatenate((Vis_data,noStim_data),1)
...
np.save(os.path.join(save_path,'PRED_StimNoStim_Vis_animal_%d' %i + '_SVM.npy'),predicted)
np.save(os.path.join(save_path,'TEST_StimNoStim_Vis_animal_%d' %i + '_SVM.npy'),y_test)
...
np.save(os.path.join(save_path,'PRED_StimNoStim_Vis_random_animal_%d' %i + '_SVM.npy'),predicted)
np.save(os.path.join(save_path,'TEST_StimNoStim_Vis_random_animal_%d' %i + '_SVM.npy'),y_test)