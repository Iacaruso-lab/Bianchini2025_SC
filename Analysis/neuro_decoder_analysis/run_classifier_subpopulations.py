...
if only_delay ==1:
    ...
    file =''.join([this_path,'PRED_mod_%d' %m + '_SVM.npy'])             
    np.save(file,predicted) # the predicted labels

    file =''.join([this_path,'TEST_mod_%d' %m +'_SVM.npy'])             
...
if m == 0:
    ...
    # what do i want to save? 
    file =''.join([this_path,'PRED_mod_delay_SVM.npy'])             
    np.save(file,predicted) # the predicted labels

    file =''.join([this_path,'TEST_mod_delay_SVM.npy'])             
    np.save(file,y_test) # the test labels    
...
if m == 0:
    ...
    for t in tqdm(range(n_iter)):
        ...
        for n in tqdm(this_n): # select increasing amount of neurons at each iteration    
            ...
            predicted[t,n,:,:], y_test[t,n,:,:]= run_clf_kfold(n_comp, scores, all_labels,k_fold = k_fold, classifier ='SVM')
    # what do i want to save? 
    file =''.join([this_path,'PRED_mod_3NOdelay_SVM.npy'])             
    np.save(file,predicted) # the predicted labels

    file =''.join([this_path,'TEST_mod_3NOdelay_SVM.npy'])             
    np.save(file,y_test) # the test labels    
