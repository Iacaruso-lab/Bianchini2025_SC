...
for n_neurons in neurons:
    ...
    for steps in actual_steps:
        ...
        np.save(''.join([this_path,'{axis}_clf_PRED_n{n_neurons}_step{step_size}_SVM.npy']),predicted) # the predicted labels
        np.save(''.join([this_path,'{axis}_clf_TEST_n{n_neurons}_step{step_size}_SVM.npy']),y_test) # the test labels   
        np.save(''.join([this_path,'{axis}_clf_IDS_n{n_neurons}_step{step_size}_SVM.npy']),specs_neurons) # the ids of the neurons
        np.save(''.join([this_path,'{axis}_clf_BINS_n{n_neurons}_step{step_size}_SVM.npy']),specs_bins) # the ids of the neurons # the number of session and animals in the bin 
...
# also do the random ones
    ...
    # what do i want to save?             
    np.save(''.join([this_path,'{axis}_clf_PRED_n{n_neurons}_random_SVM.npy']),predicted) # the predicted labels
    np.save(''.join([this_path,'{axis}_clf_TEST_n{n_neurons}_random_SVM.npy']),y_test) # the test labels   
    np.save(''.join([this_path,'{axis}_clf_IDS_n{n_neurons}_random_SVM.npy']),specs_neurons) # the ids of the neurons
