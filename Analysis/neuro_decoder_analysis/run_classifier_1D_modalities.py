...
peaks=DAT['peaks']
peaks = np.squeeze(peaks[good_pos])
...
new_modality = np.copy(modality[good_pos])
new_modality[new_modality==5] = 0
new_modality[new_modality==6] = 0
new_modality[new_modality>=3] = 3
new_modality[sig_del] = 4

#%%
# here I decide how many neurons to take
neurons =[25,50,75,100]
n_output = int(all_spikes.shape[1]/k_fold)
which_mod = np.unique(new_modality)

for m in which_mod[1:]:
    for n_neurons in neurons:
        ...
        for steps in actual_steps:
            ...
            for t in range(tot_bins): #iterate through all the bins
                ...
                # only the ones of this modality 
                curr_spikes = np.intersect1d(curr_spikes,np.where(new_modality==m)[0])
            ...
            np.save(''.join([this_path,'{axis}_clf_PRED_n{n_neurons}_step{step_size}_mod{m}_SVM.npy']),predicted) # the predicted labels
            np.save(''.join([this_path,'{axis}_clf_TEST_n{n_neurons}_step{step_size}_mod{m}_SVM.npy']),y_test) # the test labels   
            np.save(''.join([this_path,'{axis}_clf_IDS_n{n_neurons}_step{step_size}_mod{m}_SVM.npy']),specs_neurons) # the ids of the neurons
            np.save(''.join([this_path,'{axis}_clf_BINS_n{n_neurons}_step{step_size}_mod{m}_SVM.npy']),specs_bins) # the ids of the neurons # the number of session and animals in the bin 
