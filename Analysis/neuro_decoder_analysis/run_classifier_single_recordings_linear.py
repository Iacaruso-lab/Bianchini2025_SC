...
np.save(''.join([this_path,f'rec{rec}_PRED_linear_SVM.npy']),predicted) # the predicted labels
np.save(''.join([this_path,f'rec{rec}_TEST_linear_SVM.npy']),y_test) # the test labels   

# Save ids_tracking to a file
with open(''.join([this_path,f'clf_single_rec/rec{rec}_ids_tracking_linear_SVM.pkl']), 'wb') as f:
...
# save the results
np.save(''.join([this_path,f'rec{rec}_PRED_random_linear_SVM.npy']),predicted) # the predicted labels
np.save(''.join([this_path,f'rec{rec}_TEST_random_linear_SVM.npy']),y_test) # the test labels   